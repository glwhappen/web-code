...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const handleNextStep = async () => {
    ...
    if (!gitName.trim() || !gitEmail.trim()) {
      setErrorMessage(t('onboarding.validation.gitRequired'));
      return;
    }

    if (!gitEmailPattern.test(gitEmail)) {
      setErrorMessage(t('onboarding.validation.invalidEmail'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const message = await readErrorMessageFromResponse(response, t('onboarding.validation.saveGitFailed'));
        throw new Error(message);
      }

      setCurrentStep((previous) => previous + 1);
    } catch (caughtError) {
      setErrorMessage(caughtError instanceof Error ? caughtError.message : t('onboarding.validation.saveGitFailed'));
    } finally {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = await readErrorMessageFromResponse(response, t('onboarding.validation.completeFailed'));
        throw new Error(message);
      }

      await onComplete?.();
    } catch (caughtError) {
      setErrorMessage(caughtError instanceof Error ? caughtError.message : t('onboarding.validation.completeFailed'));
    } finally {
      setIsSubmitting(false);
    }
  };
