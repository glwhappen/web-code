...
function validateSetupForm(formState: SetupFormState, t: (key: string) => string): string | null {
  if (!formState.username.trim() || !formState.password || !formState.confirmPassword) {
    return t('register.validation.requiredFields');
  }

  if (formState.username.trim().length < 3) {
    return t('register.validation.usernameTooShort');
  }
...
export default function SetupForm() {
  ...
  return (
    <AuthScreenLayout
      title={t('register.setup.title')}
      description={t('register.setup.description')}
      footerText={t('register.setup.footerText')}
